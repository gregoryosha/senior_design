...
const int RotaryCLK = 9; //CLK pin on the rotary encoder
const int RotaryDT = 12; //DT pin on the rotary encoder
const int PushButton = 13; //Button to enter/exit menu
...
void setup()
{
  ...
  lcd.print("Shaft Cutter");
  lcd.setCursor(0, 1); //Second row, first column
  lcd.print("Version 2");
  lcd.setCursor(0, 2); //Second row, first column
  lcd.print("by Greg Chris Gabe");
...
void loop()
{
  ...
  if (btnState == LOW && pressed == false) {
    //if 50ms have passed since last LOW pulse, it means that the
    //button has been pressed, released and pressed again
      pushButton();
      pressed = true;
  }
  else if (btnState == HIGH && pressed == true) {
    pressed = false;
  }
